package ma.zs.stocky.service.impl.encadrant.stage;
...
import ma.zs.stocky.service.facade.encadrant.stage.StageEncadrantService;
...
import ma.zs.stocky.service.facade.encadrant.societe.SocieteEncadrantService ;
import ma.zs.stocky.bean.core.societe.Societe ;
import ma.zs.stocky.service.facade.encadrant.stage.StageEtudiantEncadrantService ;
import ma.zs.stocky.bean.core.stage.StageEtudiant ;
import ma.zs.stocky.service.facade.encadrant.jury.JuryEncadrantService ;
import ma.zs.stocky.bean.core.jury.Jury ;
import ma.zs.stocky.service.facade.encadrant.departement.FiliereEncadrantService ;
import ma.zs.stocky.bean.core.departement.Filiere ;
import ma.zs.stocky.service.facade.encadrant.stage.StageEncadrantExterneEncadrantService ;
import ma.zs.stocky.bean.core.stage.StageEncadrantExterne ;
import ma.zs.stocky.service.facade.encadrant.stage.StagePiecesAttachementEncadrantService ;
import ma.zs.stocky.bean.core.stage.StagePiecesAttachement ;
import ma.zs.stocky.service.facade.encadrant.stage.StageEncadrantInterneEncadrantService ;
import ma.zs.stocky.bean.core.stage.StageEncadrantInterne ;
import ma.zs.stocky.service.facade.encadrant.stage.TypeStageEncadrantService ;
...
import java.util.List;
@Service
public class StageEncadrantServiceImpl implements StageEncadrantService {
    ...
    public void deleteAssociatedLists(Long id) {
        ...
        stagePiecesAttachementService.deleteByStageId(id);
    }
    ...
    public Stage create(Stage t) {
        ...
        if (loaded == null) {
            ...
            if (t.getStagepiecesAttachement() != null) {
                t.getStagepiecesAttachement().forEach(element-> {
                    element.setStage(saved);
                    stagePiecesAttachementService.create(element);
                });
            }
        }else {
            saved = null;
    ...
    public Stage findWithAssociatedLists(Long id){
        ...
        if(result!=null && result.getId() != null) {
            ...
            result.setStagepiecesAttachement(stagePiecesAttachementService.findByStageId(id));
        }
        return result;
    }
    ...
    if(stage !=null && stage.getId() != null){
        ...
        List<List<StagePiecesAttachement>> resultStagepiecesAttachement= stagePiecesAttachementService.getToBeSavedAndToBeDeleted(stagePiecesAttachementService.findByStageId(stage.getId()),stage.getStagepiecesAttachement());
            stagePiecesAttachementService.delete(resultStagepiecesAttachement.get(1));
        ListUtil.emptyIfNull(resultStagepiecesAttachement.get(0)).forEach(e -> e.setStage(stage));
        stagePiecesAttachementService.update(resultStagepiecesAttachement.get(0),true);
        }
    }
    ...
    @Autowired
    private SocieteEncadrantService societeService ;
    @Autowired
    private StageEtudiantEncadrantService stageEtudiantService ;
    @Autowired
    private JuryEncadrantService juryService ;
    @Autowired
    private FiliereEncadrantService filiereService ;
    @Autowired
    private StageEncadrantExterneEncadrantService stageEncadrantExterneService ;
    @Autowired
    private StagePiecesAttachementEncadrantService stagePiecesAttachementService ;
    @Autowired
    private StageEncadrantInterneEncadrantService stageEncadrantInterneService ;
    @Autowired
    private TypeStageEncadrantService typeStageService ;
    @Autowired
